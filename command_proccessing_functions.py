...
def check_cur_offers(src, category_id, cur_offers = None, return_bool = False):
    ...
    for offer in offers:
        ...
        if cur_date <= finish_date and cur_date >= start_date and (not str(offer[0]) in users_db.get_offers_taken(src.from_user.id)) and requests_db.check_if_users_offer_exists(src.from_user.id, offer[0]):
            if return_bool:
                is_actual = True
                break
            if cur_offers != None:
