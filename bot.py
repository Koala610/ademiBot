...
async def add_date(message : types.Message,state:FSMContext):
     ...
     users_db.update_date(message.from_user.id, datetime.date(date[0], date[1], date[2]))
     if await check_full_fields(message) == True:
        await show_succ_message(message, state = state)
...
async def show_more_info(callback_query: types.CallbackQuery):
    ...
    result += offer['text'] + '\n'
    result += "Начало: " '<i>'+str(offer['start_date']) + ' ' + str(offer['start_time']) + '</i>' '\n'
    result += "Конец: " '<i>'+ str(offer['finish_date']) + ' ' + str(offer['end_time']) + '</i>' '\n'
